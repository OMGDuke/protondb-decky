  ...
  const badge = protonData ? (
    <DialogButton
      ...
      style={{
        ...
        left: 20,
        position: 'absolute',
        background: (tierColours[protonData?.tier] || tierColours.none)
          .background,
        zIndex: 20,
    ...
    >
      <img
        ...
        style={{
          ...
          filter: `brightness(${
            (
              tierLogoChanges[protonData?.tier as 'borked'] ||
      ...
      <span style={{ marginLeft: 10, fontSize: 20 }}>
        {`${protonData?.tier?.charAt(0).toUpperCase()}${protonData?.tier?.slice(
          1
        )}`}
      </span>
    </DialogButton>
