...
export default function ProtonMedal({
  ...
  const badge = protonDBTier ? (
    ...
    >
      {linuxSupport ? <IoLogoTux size={20} /> : <></>}
      <img
        alt="ProtonDB Logo"
        style={{
          marginLeft: linuxSupport ? 10 : 0,
          width: '20px',
          filter: `brightness(${
            (tierLogoChanges[protonDBTier as 'borked'] || tierLogoChanges.none)
              .brightness
          })
                  invert(${
                    (
                      tierLogoChanges[protonDBTier as 'borked'] ||
                      tierLogoChanges.none
                    ).invert
                  })`
        }}
